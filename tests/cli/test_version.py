...
class TestVersionFunctions(unittest.TestCase):
    @patch("ts_backend_check.cli.version.importlib.metadata.version")
    def test_get_local_version_installed(self, mock_version):
        mock_version.return_value = "1.0.0"
        self.assertEqual(get_local_version(), "1.0.0")

    @patch(
        "ts_backend_check.cli.version.importlib.metadata.version",
    ...
    @patch("ts_backend_check.cli.version.get_local_version", return_value="X.Y.Z")
    @patch(
        "ts_backend_check.cli.version.get_latest_version",
        return_value="ts-backend-check X.Y.Z",
    )
    def test_get_version_message_up_to_date(
        ...
        """
        expected_message = "ts-backend-check vX.Y.Z"
        self.assertEqual(get_version_message(), expected_message)

    @patch("ts_backend_check.cli.version.get_local_version", return_value="X.Y.Y")
    @patch(
        "ts_backend_check.cli.version.get_latest_version",
        return_value="ts-backend-check X.Y.Z",
    )
    def test_upgrade_available(self, mock_latest_version, mock_local_version):
        ...
        """
        expected_message = "ts-backend-check vX.Y.Y (Upgrade available: ts-backend-check vX.Y.Z). To upgrade: ts-backend-check -u"
        self.assertEqual(get_version_message(), expected_message)

    @patch(
        "ts_backend_check.cli.version.get_local_version",
