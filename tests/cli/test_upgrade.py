...
from ts_backend_check.cli.version import UNKNOWN_VERSION_NOT_FETCHED
...
class TestUpgradeCLI:
    ...
    @patch("ts_backend_check.cli.upgrade.get_local_version")
    @patch("ts_backend_check.cli.upgrade.get_latest_version")
    ...
    def test_upgrade_cli_already_latest_version(
        ...
        mock_get_latest.return_value = "ts-backend-check v5.1.0"

        upgrade_cli()

        mock_print.assert_called_once_with(
            "You already have the latest version of ts-backend-check."
        )
    ...
    def test_upgrade_cli_local_version_higher(
        ...
        expected_message = (
            "ts-backend-check v5.2.0 is higher than the currently released version ts-backend-check v5.1.0. "
            "Hopefully this is a development build, and if so, thanks for your work on ts-backend-check! "
            "If not, please report this to the team at https://github.com/activist-org/ts-backend-check/issues."
        )
        mock_print.assert_called_once_with(expected_message)

    @patch("ts_backend_check.cli.upgrade.subprocess.check_call")
    @patch("ts_backend_check.cli.upgrade.get_local_version")
    @patch("ts_backend_check.cli.upgrade.get_latest_version")
    @patch("builtins.print")
    def test_upgrade_cli_successful_upgrade(
        ...
        expected_calls = [
            ...
            call("Updating ts-backend-check with pip..."),
        ]
        mock_print.assert_has_calls(expected_calls)

        mock_check_call.assert_called_once_with(
            [sys.executable, "-m", "pip", "install", "--upgrade", "ts-backend-check"]
        )
    ...
    def test_upgrade_cli_subprocess_error(
        ...
        assert "Failed to install the latest version of ts-backend-check" in str(
            error_calls[0]
        )
    ...
    def test_upgrade_cli_string_comparison_edge_case(
        ...
        mock_get_latest.return_value = "ts-backend-check v5.2.0"

        upgrade_cli()

        # With proper semantic versioning, 5.10.0 > 5.2.0 so it shows higher version message.
        expected_message = (
            "ts-backend-check v5.10.0 is higher than the currently released version ts-backend-check v5.2.0. "
            "Hopefully this is a development build, and if so, thanks for your work on ts-backend-check! "
            "If not, please report this to the team at https://github.com/activist-org/ts-backend-check/issues."
        )
        mock_print.assert_called_once_with(expected_message)
    ...
    def test_upgrade_cli_proper_higher_version_scenario(
        ...
        expected_message = (
            "ts-backend-check v6.0.0 is higher than the currently released version ts-backend-check v5.1.0. "
            "Hopefully this is a development build, and if so, thanks for your work on ts-backend-check! "
            "If not, please report this to the team at https://github.com/activist-org/ts-backend-check/issues."
        )
        mock_print.assert_called_once_with(expected_message)
    ...
    def test_upgrade_cli_semantic_version_upgrade_needed(
        ...
        with patch(
            "ts_backend_check.cli.upgrade.subprocess.check_call"
        ) as mock_check_call:
            mock_check_call.return_value = None
            upgrade_cli()
